<?php
/**
 * Main Century ToolKit plugin class/file.
 *
 * @package centurytoolkit
 */


/**
 * Century ToolKit class, so we don't have to worry about namespaces.
 */
class TCY_Main{
	 ...
	 *
	 * @return TCY_Main the *Singleton* instance.
	 */
	...
	public function display_plugin_page() {
        ...
        <div class="wrap">
			...
			if ( ini_get( 'safe_mode' ) ) {
				printf(
					esc_html__( '%sWarning: your server is using %sPHP safe mode%s. This means that you might experience server timeout errors.%s', 'century-toolkit' ),
		...
		if ( $this->plugin_page === $hook ) {
			...
			wp_localize_script( 'mg-main-js', 'mg',
				array(
					...
					'texts'        => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'century-toolkit' ),
					),
				)
			);
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			$demo_import_start_time = date( apply_filters( 'century-toolkit/date_format_for_file_names', 'Y-m-d__H-i-s' ) );

			// Define log file path.
			$this->log_file_path = TCY_Helpers::get_log_path( $demo_import_start_time );
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->selected_import_files = TCY_Helpers::process_uploaded_files( $_FILES, $this->log_file_path );
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					TCY_Helpers::log_error_and_send_ajax_response(
						$this->selected_import_files->get_error_message(),
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'century-toolkit' )
					);
				}
				...
				$log_added = TCY_Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'century-toolkit' ),
						$this->import_files[ $this->selected_index ]['import_file_name']
					) . TCY_Helpers::import_file_info( $this->selected_import_files ),
					$this->log_file_path,
					esc_html__( 'Downloaded files', 'century-toolkit' )
				);
			} else {
				...
				wp_send_json( esc_html__( 'No import files specified!', 'century-toolkit' ) );
			}
		}
		 ...
		 */
		$action = 'century-toolkit/before_widgets_import';
		if ( ( false !== has_action( $action ) ) && empty( $this->frontend_error_messages ) ) {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			$response['message'] = sprintf(
				__( 'The demo import has finished. Please check your page and make sure that everything has imported correctly.', 'century-toolkit' ) );

		} else {
			$response['message'] = $this->frontend_error_messages . '<br>';
			$response['message'] .= sprintf(
				__( 'The demo import has finished, but there were some import errors.%s More details about the errors can be found in this %s %slog file%s %s.', 'century-toolkit' ),
				'<br>',
				'<strong>',
				'<a href="' . TCY_Helpers::get_log_url( $this->log_file_path ) . '" target="_blank">',
				'</strong>',
				'</a>'
			);
		}
	...
	function import_content(
		...
		if ( ! apply_filters( 'century-toolkit/regenerate_thumbnails_in_content_import', true ) ) {
		...
		if ( ! empty( $import_file_path ) ) {
			...
			// Add this message to log file.
			$log_added = TCY_Helpers::append_to_file(
				$message . PHP_EOL . esc_html__( 'Max execution time after content import = ', 'century-toolkit' ) . ini_get( 'max_execution_time' ),
				$this->log_file_path,
				esc_html__( 'Importing content', 'century-toolkit' )
			);
		}

		// Delete content importer data for current import from DB.
		delete_transient( 'TCY_importer_data' );
	...
	function import_widgets(
		...
		$widget_importer = new TCY_Importer_Widget();
		...
		if ( is_wp_error( $results ) ) {
			...
			TCY_Helpers::log_error_and_send_ajax_response(
				...
				$this->log_file_path,
				esc_html__( 'Importing widgets', 'century-toolkit' )
			);
		}
	...
	function import_customizer(
		...
		if ( is_wp_error( $results ) ) {
			...
			TCY_Helpers::log_error_and_send_ajax_response(
				$results->get_error_message(),
				$this->log_file_path,
				esc_html__( 'Importing customizer settings', 'century-toolkit' )
			);
		}

		// Add this message to log file.
		$log_added = TCY_Helpers::append_to_file(
			esc_html__( 'Customizer settings import finished!', 'century-toolkit' ),
			$this->log_file_path,
			esc_html__( 'Importing customizer settings', 'century-toolkit' )
		);
	}
	...
	public function new_ajax_request_maybe(
		...
		if ( $time > apply_filters( 'century-toolkit/time_for_one_ajax_call', 25 ) ) {
	...
	function set_importer_data() {
		...
		set_transient( 'TCY_importer_data', $data, 0.5 * HOUR_IN_SECONDS );
	}
	...
	private function get_importer_data() {
		if ( $data = get_transient( 'TCY_importer_data' ) ) {
	...
	function setup_plugin_with_filter_data() {
		...
		$logger_options = apply_filters( 'century-toolkit/logger_options', array(
			'logger_min_level' => 'warning',
		) );

		// Configure logger instance and set it to the importer.
		$this->logger            = new TCY_Logger();
		$this->logger->min_level = $logger_options['logger_min_level'];

		//// Create importer instance with proper parameters.
		$this->importer = new TCY_Importer_Main( $importer_options, $this->logger );
	}
}